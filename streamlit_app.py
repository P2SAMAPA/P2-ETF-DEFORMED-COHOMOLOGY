...
st.set_page_config(page_title="Deformed Cohomology – Topological Field Theory", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Deformed Cohomology Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Topological field theory | Macro‑deformed Betti‑1 derivative as per‑ETF score</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Deformed Cohomology")
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Rips max dist:** {config.RIPS_MAX_DISTANCE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'deformed_cohomology_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Deform score: {etf['deform_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Deform Score"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Deform Score", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Deformed Cohomology Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Deformed cohomology** studies how topological invariants (Betti numbers) change under a deformation parameter.
        - We use the macro variable (e.g., VIX) as the deformation parameter, scaling the distance matrix.
        - The score is the derivative of Betti-1 (number of loops) with respect to macro, measured by the change in loops when each ETF is removed.
        - High score → ETF's removal significantly changes the loop structure under macro deformation → structurally important.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the deformed cohomology scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Deformed Cohomology | Topological field theory with macro deformation")
